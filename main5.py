        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if l[4].startswith(l[3]):
                    ...
                    game_over-=1
                    l=Easy_word()
                    Other_screen(game_over,score,background,mode,start_time)
                    # yourword=''
                elif game_over==0:
                    Other_screen(game_over,score,background,mode,start_time)
                    time.sleep(2)
...
def Other_screen(game_over,score,background,mode,start_time):
    gameDisplay.blit(background, (0,0))
    if game_over==0 :
        draw_text(gameDisplay, "GAME OVER!", 90, WIDTH / 2, HEIGHT / 4)
        draw_text(gameDisplay,"Final Score : " + str(score), 70, WIDTH / 2, HEIGHT /2)
        end_time= time.time()
        draw_text(gameDisplay,"Typing speed : "+str(round(score*60/((end_time-start_time)*5)))+" wpm", 70, WIDTH / 2, HEIGHT-150)
    elif game_over==4:
